import { pgTable, pgEnum, uuid, text , date, boolean, integer, real, varchar } from "drizzle-orm/pg-core";

export const userRole = pgEnum("user_role", ["admin", "buyer", "shoper"]);

export const orderStatus = pgEnum("order_status", ["pending_payment", "paid", "ready_to_ship", "cancelled", "delivered"]);
...
export const cartItems = pgTable("cart_items", {
  ...
  productId: uuid("product_id").references(() => products.id, { onDelete: "cascade" }),
  variantId: uuid("variant_id").references(() => productVariants.id, { onDelete: "cascade" }),
  variantChoiceId: uuid("variant_choice_id").references(() => productVariantChoices.id, { onDelete: "cascade" }),
});
...
export const orderItems = pgTable("order_items", {
  ...
  productId: uuid("product_id").references(() => products.id, { onDelete: "cascade" }),
  variantId: uuid("variant_id").references(() => productVariantChoices.id, { onDelete: "cascade"}),
  variantChoiceId: uuid("variant_choice_id").references(() => productVariantChoices.id, { onDelete: "cascade" }),
});
